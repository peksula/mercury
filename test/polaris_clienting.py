...
class PolarisClienting(unittest.TestCase):
    ...
    def test_sending_multi_line_string_succeeds(self, mock_post):
        return_value = self.client.send_multi_line_string(self.payload)
        mock_post.assert_called_with("fake_server_url/api/multilinestring", json=self.payload)
        self.assertTrue(return_value)

    @patch('requests.post', autospec=True)
    def test_sending_multi_line_string_fails_can_not_connect(self, mock_post):
